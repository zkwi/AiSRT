...
- 识别并翻译：生成 ASR 原始字幕后，继续输出目标语言字幕。
- 文件队列：显示文件、状态、进度和字幕目录。
- 常用设置：上下文、识别语言、输出语言、运行模式、模型尺寸。
...
- 添加文件和开始处理是核心按钮，保留图标；识别并翻译、停止和翻译字幕默认只显示文字，避免界面噪声。
...
主界面的“识别并翻译”适合从音视频一步生成双语结果。例如识别语言设为英语、输出语言设为简体中文时，会在媒体文件旁生成：
...
一键流程会先写入 ASR 原始字幕，再加载翻译模型。若翻译模型缺失或翻译失败，原始字幕会保留，队列状态会提示“翻译失败，已保留原字幕”，可在模型准备好后重试失败项。
