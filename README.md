# AI 大模型字幕助手（AISRT）- 本地 AI 字幕生成与 SRT 翻译工具

语言：简体中文 | [English](README.en.md)

AISRT 是一个本地运行的 AI 字幕生成工具，用于把视频、音频转换为带时间轴的 `.srt` 字幕。它使用本地大模型 ASR 进行多语言语音识别，结合强制对齐生成更准确的字幕时间轴，并支持本地 SRT 翻译，适合需要离线字幕生成、批量字幕处理和隐私友好工作流的个人用户。

项目定位很明确：个人桌面/命令行工具，优先保证简单、稳定、可维护。不做 Web UI，不做服务端，不接数据库。

关键词：本地 AI 字幕生成、音视频转 SRT、多语言 ASR、字幕时间轴对齐、SRT 翻译、离线字幕工具、Windows 字幕生成、GUI 字幕软件、CLI 字幕批处理。

## 界面预览

![AISRT 本地 AI 字幕生成工具主界面，支持视频音频转 SRT、多语言识别和本地翻译](docs/assets/aisrt-main-window-zh.png)
...
- 进度提示：ASR 识别和字幕翻译会显示预估剩余时间，格式为 `分钟:秒`。
- 语言设置：界面语言默认跟随系统语言，手动切换后会记住；识别字幕语言用于 ASR；翻译语言用于生成译文字幕。
...
- 支持简体中文、繁体中文、英语、日语、韩语、西班牙语六种界面语言；首次启动默认匹配系统语言，手动切换后优先使用用户选择。
...
1. 选择已有 `.srt` 文件。
2. 选择翻译语言，GUI 只展示识别与翻译共同支持的常用语种。
...
| `--language` | 识别语言，默认 `auto`；GUI 常用预设包括 `English`、`Chinese`、`Japanese`、`Korean`、`Spanish`、`French`、`German` 等，CLI 可传入模型支持的语言名。 |
...
- 明确指定识别字幕语言。
- CLI 可通过 `-c, --context` 提供片名、人名、地名或专有名词。
- 把音频分块从 45 秒改为 30 秒。
- 使用 1.7B 模型而不是 0.6B。
