...
- 启用翻译：勾选后，“开始处理”会先生成 ASR 原始字幕，再继续输出目标语言字幕。
- 文件队列：显示文件、状态、进度和字幕目录。
- 常用设置：上下文、识别语言、是否启用翻译、目标语言、运行模式、模型尺寸。
...
| 启用翻译 | 关闭 | 需要同时生成目标语言字幕时再勾选；会保留原始字幕。 |
| 运行模式 | 推荐 | 显存不足时切换到低显存。 |
...
主界面勾选“启用翻译”后，适合从音视频一步生成双语结果。例如识别语言设为英语、目标语言设为简体中文时，会在媒体文件旁生成：
