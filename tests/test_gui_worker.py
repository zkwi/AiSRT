...
def test_subtitle_worker_translates_after_asr_and_keeps_original(tmp_path, monkeypatch) -> None:
    ...
    assert any("翻译字幕 100%" in detail for _percent, detail in progress)
    assert progress[-1] == (100, "已处理 1/1")
    assert statuses[-1] == (0, "完成", str(tmp_path))
    assert finished == [True]


def test_subtitle_worker_keeps_original_when_translation_load_fails(tmp_path, monkeypatch) -> None:
    import aisrt.gui_worker as worker_module
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: (_ for _ in ()).throw(RuntimeError("missing translation model")),
    )
    ...
    assert parse_srt(original.read_text(encoding="utf-8"))[0].text == "Hello, world."
    assert statuses[-1] == (0, "翻译失败，已保留原字幕", str(tmp_path))
    assert any("翻译失败，已保留原始字幕: missing translation model" in log for log in logs)
    assert any(detail.endswith("加载翻译模型") for _percent, detail in progress)
