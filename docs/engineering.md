# AISRT 工程治理 Wiki：本地字幕生成工具的模块边界与 GUI/CLI 约定

AISRT 是个人使用的本地 AI 字幕生成工具，支持多语言音视频转 SRT、本地 SRT 翻译和 GUI/CLI 两种使用方式。治理目标是“稳定可用、容易回看、改动可控”。不追求企业级流程，但每次变更都应留下可验证的依据。
...
- 主界面按钮图标保持克制：只在“添加文件”和“开始处理”两个核心操作上使用图标；翻译开关、翻译已有 SRT、高级设置、停止和弹窗按钮默认只显示文字。
- GUI 多语言文案集中在 `gui_i18n.py`。新增用户可见字段时必须补齐当前 `UI_LANGUAGE_OPTIONS` 中的所有界面语言，并更新 `tests/test_gui_window.py`。
- GUI 主界面只放高频操作；上下文提示保留给 CLI，设备、音频分块和字幕样式放入高级设置弹窗，避免大字号和高 DPI 下挤压常用设置。模型路径和对齐模型属于内部细节，不在 GUI 中暴露给普通用户。
- 运行模式负责在后台调整批大小、token、dtype 等技术参数，不把这些技术细节重新放回界面。
- 语言相关控件必须明确区分：界面语言只影响 GUI 文案，识别字幕语言用于 ASR，翻译语言用于输出译文字幕。识别字幕语言默认自动识别；新增语言选项时应使用底层 ASR 支持的规范英文语言名作为内部值。
...
- 主界面通过“启用翻译”复选框把本地翻译自然并入处理流程：ASR 成功后保留 `movie.srt` 原始识别字幕，再生成 `movie.<语言后缀>.srt` 翻译字幕。覆盖检查必须同时覆盖两个输出文件；翻译模型加载失败或翻译失败时不得删除或阻断已生成的原始字幕。
